...
export function useRecordingStop(
  ...
  const handleRecordingStop = useCallback(async (isCallApi: boolean) => {
    ...
    try {
      ...
      if (isTranscribeLive) {
        // NOTE: By the time we get here, `invoke('stop_recording')` (called by
        // RecordingControls.stopRecordingAction before onRecordingStop) has already
        // awaited the Rust-side transcription task to completion with no timeout
        // (see recording_commands.rs::stop_recording) — every transcript-update event
        // has already been emitted. There is nothing left to poll for; the old status
        // poll here queried a stub Tauri command (`get_transcription_status`) that never
        // reflected the real queue, so it always "completed" on its first check and only
        // added artificial delay. We keep a short fixed wait purely to let the last
        // transcript-update events finish propagating into React state before flushing.
        ...
        // "Record only" mode: no live transcript-update events ever fired, so there's
        // nothing to wait for or flush. The meeting is saved audio-only below and can
        // be transcribed later on demand from meeting-details ("Transcribe" button).
        console.log('🔇 Record-only mode: skipping transcription wait/flush');
      }
      ...
      if (isCallApi) {
        ...
        // Get fresh transcript state (ALL transcripts including late ones). Record-only
        // mode never received any transcript-update events, so this is empty by
        // construction - saved explicitly as 'pending' so meeting-details offers the
        // "Transcribe" action instead of showing an empty transcript.
        const freshTranscripts = isTranscribeLive ? [...transcriptsRef.current] : [];
        ...
        try {
          const responseData = await storageService.saveMeeting(
            ...
            freshTranscripts,
            folderPath,
            isTranscribeLive ? undefined : 'pending'
          );
